...
3. **Build the Store packages on Windows.** With those exact values supplied only to the build environment, run `npm run msix:prepare`, `npm run dist:win:store:x64`, and `npm run dist:win:store:arm64` from `desktop/`. The resolver must reject blank or placeholder values.[^readiness]
