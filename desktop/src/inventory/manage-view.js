  ...
  if (row.state === 'duplicate') {
    if (row.resolution) {
      ...
      const detail = needsChoice
        ...
        : `These copies are identical. CCTI keeps the copy saved for ${options[keeper]}; Resolve removes the extra ${options.length - 1 === 1 ? 'copy' : 'copies'}, so nothing stops working.`;
      return {
