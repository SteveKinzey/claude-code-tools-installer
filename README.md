...
- **In-app manifest verifier, drag-and-drop, and command copy:** Users can choose or drag a saved manifest onto the verification utility, or copy a platform-native terminal command (`shasum -a 256` / `Get-FileHash`) to verify the file independently outside the app.
- **Verified manifest diff:** Users can choose two saved manifests to see the verified tool names added or removed between exports. CCTI refuses the comparison if either manifest checksum fails.
- **Release integrity alert:** The existing background release check shows a live diagnostics pill whenever a published GitHub release artifact lacks its SHA-256 digest. It does not download or install anything automatically.
